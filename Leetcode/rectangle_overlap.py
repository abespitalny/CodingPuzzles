'''
A rectangle is represented as a list [x1, y1, x2, y2], where (x1, y1) are the coordinates of its bottom-left corner,
and (x2, y2) are the coordinates of its top-right corner.

Two rectangles overlap if the area of their intersection is positive.
To be clear, two rectangles that only touch at the corner or edges do not overlap.

Given two (axis-aligned) rectangles, return whether they overlap.
'''
from leetcode import *

# Time: O(1), Space: O(1).
def is_rectangle_overlap(rec1: List[int], rec2: List[int]) -> bool:
    # if (rec2[0] >= rec1[2]) or (rec2[1] >= rec1[3]):
    #     return False
    # if (rec2[2] <= rec1[0]) or (rec2[3] <= rec1[1]):
    #     return False
    # return True

    # A simplification of the above code, but the code above explains exactly the reasoning of this one-liner.
    return ((rec2[0] < rec1[2]) and (rec2[1] < rec1[3])) and ((rec2[2] > rec1[0]) and (rec2[3] > rec1[1]))


# Expected: True
print(is_rectangle_overlap([0,0,2,2], [1,1,3,3]))
# Expected: False
print(is_rectangle_overlap([0,0,1,1], [1,0,2,1]))
# Expected: True
print(is_rectangle_overlap([7,8,13,15], [10,8,12,20]))
# Expected: True
print(is_rectangle_overlap([0,0,1,1], [0,0,1,2]))
