...
class Node:
    ...
    def __repr__(self):
        return '{color} {val} : {data} Node'.format(color=self.color, val=self.value, data=self.data)

    def __iter__(self):
        if self.left.color is not None:
            yield from self.left.__iter__()

        yield self.value

        if self.right.color is not None:
            yield from self.right.__iter__()

    def __eq__(self, other):
        if self.color is None and self.color == other.color:
            return True
    ...
    def get_children_count(self) -> int:
        ...
        if self.color is None:
            return 0
        return sum([int(self.left.color is not None), int(self.right.color is not None)])


class RedBlackTree:
    # every node has null nodes as children initially, create one such object for easy management
    NIL_LEAF = Node(value=None, color=None, data=None, parent=None)
    ...
    def _find_parent(self, value):
        ...
        def inner_find(parent):
            ...
            if value == parent.value:
                ...
                if parent.right.color is None:  # no more to go
                    return parent, 'R'
                return inner_find(parent.right)
            elif value < parent.value:
                if parent.left.color is None:  # no more to go
                    return parent, 'L'
